# Useful Links

<img src="useful-links.png"  width="50%">

<!-- @import "[TOC]" {cmd="toc" depthFrom=1 depthTo=6 orderedList=false} -->

<!-- code_chunk_output -->

- [Useful Links](#useful-links)
  - [API](#api)
    - [General Guidance](#general-guidance)
      - [RFC 7807: Problem Details for HTTP APIs](#rfc-7807-problem-details-for-http-apis)
    - [APIs](#apis)
    - [REST](#rest)
      - [Asynchronous API](#asynchronous-api)
      - [Synchronous API](#synchronous-api)
        - [OpenAPI](#openapi)
      - [Cloud Events](#cloud-events)
      - [Versioning](#versioning)
  - [Python](#python)
    - [API'sin Python](#apisin-python)
  - [Security](#security)
  - [IDSR](#idsr)
  - [OpenADR](#openadr)
  - [Other](#other)
    - [Diagrams](#diagrams)
    - [Markdown](#markdown)
    - [Architecture](#architecture)
      - [ADR](#adr)
    - [Topics (MQTT)](#topics-mqtt)
    - [GCP](#gcp)
    - [GUID](#guid)
  - [Examples](#examples)
    - [API's](#apis-1)
  - [Products](#products)
  - [IOT](#iot)
    - [MQTT](#mqtt)
    - [MQTT IoT](#mqtt-iot)
    - [IoT Mobile](#iot-mobile)
    - [IoT HW](#iot-hw)
    - [IoT platforms](#iot-platforms)
  - [Top Tools](#top-tools)
    - [Design tools](#design-tools)
    - [API](#api-1)
    - [WSL](#wsl)
    - [Linux tools](#linux-tools)
    - [Other](#other-1)
  - [Development methodologies](#development-methodologies)
    - [SAFe](#safe)
  - [Other (temp dumnping ground for links)](#other-temp-dumnping-ground-for-links)

<!-- /code_chunk_output -->
## API

### General Guidance

- [REST API Design](https://www.mscharhag.com/p/rest-api-design)
- [API Design Principles](https://linked.art/api/1.0/principles/)
- [Best Practices for API Error Handling](https://nordicapis.com/best-practices-api-error-handling/)
- [API related blogs](https://nordicapis.com/blog/)
- [RESTful web API design](https://learn.microsoft.com/en-us/azure/architecture/best-practices/api-design)
- [Best Practices in API Design](https://swagger.io/resources/articles/best-practices-in-api-design/)
- [8 Reasons why architects love API driven architecture](https://hub.packtpub.com/architects-love-api-driven-architecture/)
- [THE BENEFITS OF API-DRIVEN DEVELOPMENT AND HOW YOU CAN IMPLEMENT IT](https://www.mertech.com/blog/api-driven-development)

#### RFC 7807: Problem Details for HTTP APIs

- [RFC 7807](https://www.rfc-editor.org/rfc/rfc7807)
- [RFC 7807: error handling standard for APIs](https://www.linkedin.com/pulse/rfc-7807-error-handling-standard-apis-david-rold%C3%A1n-mart%C3%ADnez/)

### APIs

### REST

- [Kenneth Lange Lttle book on REST](http://www.kennethlange.com/books/The-Little-Book-on-REST-Services.pdf&usg=AOvVaw1cqSsJE4cRxIDf6ieavWyI)
- [REST API Tutorial](https://www.restapitutorial.com/)
- [RESTful API Design: nouns are good, verbs are bad | Google Cloud Blog](https://cloud.google.com/blog/products/api-management/restful-api-design-nouns-are-good-verbs-are-bad)

#### Asynchronous API

- [AsyncAPI](https://www.asyncapi.com/)

#### Synchronous API

##### OpenAPI

- [OpenAPI](https://www.openapis.org/)
- [Introduction - OpenAPI Documentation (oai.github.io)](https://oai.github.io/Documentation/introduction.html)
- [OpenAPI.Tools - an Open Source list of great tools for Open API](https://openapi.tools/)
- [OpenAPI tutorial using Swagger Editor and Swagger UI: Overview | Documenting APIs (idratherbewriting.com)](https://idratherbewriting.com/learnapidoc/pubapis_openapi_tutorial_overview.html)
- [OpenAPI 3.0 Tutorial | SwaggerHub Documentation (smartbear.com)](https://support.smartbear.com/swaggerhub/docs/tutorials/openapi-3-tutorial.html)
- [openapi tutorial overview](https://idratherbewriting.com/learnapidoc/pubapis_openapi_tutorial_overview.html)
- [Units](https://idratherbewriting.com/learnapidoc/pubapis_openapi_step5_components_object.html)
- [more units](https://github.com/OAI/OpenAPI-Specification/issues/2061)
- [even more on units](https://github.com/json-schema-org/json-schema-vocabularies/issues/46)

#### Cloud Events

- [cloudevents.io](https://cloudevents.io/)
- [CloudEvents - spec](https://github.com/cloudevents/spec)
- [AsyncAPI and CloudEvents](https://www.asyncapi.com/blog/asyncapi-cloud-events)

#### Versioning

- [Semantic Versioning](https://semver.org/)
- [SemVer Github](https://github.com/semver/semver)

## Python

- [Python Tutorial](https://www.w3schools.com/python/default.asp)
- [Getting Started With Async Features in Python – Real Python](https://realpython.com/python-async-features/)

### API'sin Python

- [Python REST APIs With Flask, Connexion, and SQLAlchemy](https://dassum.medium.com/python-rest-apis-with-flask-connexion-and-sqlalchemy-3c8c3292d9ce)
- [Python REST APIs With Flask, Connexion, and SQLAlchemy – Part 3](https://realpython.com/flask-connexion-rest-api-part-3/)

## Security

- [NIST.SP.800-53r5 Security and Privacy Controls for Information Systems and Organizations](https://nvlpubs.nist.gov/nistpubs/SpecialPublications/NIST.SP.800-53r5.pdf)
- [The Network and Information Systems Regulations 2018](https://www.legislation.gov.uk/uksi/2018/506/made)
- [Cyber Assessment Framework](https://www.ncsc.gov.uk/collection/caf/cyber-assessment-framework)
- [OWASP IoT Security Verification Standard (ISVS)](https://github.com/OWASP/IoT-Security-Verification-Standard-ISVS)
- [OWASP Secure Coding Practices-Quick Reference Guide](https://owasp.org/www-project-secure-coding-practices-quick-reference-guide/)
- [OWASP Secure Coding Practices Quick Reference Guide](https://owasp.org/www-pdf-archive/OWASP_SCP_Quick_Reference_Guide_v2.pdf))
- [ETSI IoT](https://www.etsi.org/technologies/consumer-iot-security)

## IDSR

- [Interoperable Demand Side Response programme](https://www.gov.uk/government/collections/interoperable-demand-side-response-programme#:~:text=The%20Interoperable%20Demand%20Side%20Response%2cof%20interoperable%20demand%20side%20response.)
- [Interoperable Demand Side Response programme - Stream 1](https://www.gov.uk/government/publications/interoperable-demand-side-response-programme-apply-for-stream-1-opportunities)
- [Interoperable Demand Side Response programme - Stream 2](https://www.gov.uk/government/publications/interoperable-demand-side-response-programme-apply-for-stream-2-opportunities)

## OpenADR

- [ADR_InNutshell (openadr.org)](https://www.openadr.org/assets/docs/DTECH2015/what%20is%20openadr.pdf)
- [Introduction & Overview (openadr.org)](https://www.openadr.org/assets/docs/understanding%20openadr%202%200%20webinar_11_10_11_sm.pdf)
- [Microsoft Word - OpenADR_Primer_0812Final.doc](https://www.openadr.org/assets/docs/openadr_primer.pdf)
- [OpenADR Webinar Series](https://www.openadr.org/webinar-series)

## Other

### Diagrams

- [Plant UML](https://plantuml.com/)
- [Mermaid](https://mermaid.js.org)
- [sequence diagram](https://sequencediagram.org/)
- [dotUML](https://dotuml.com/)
- [Architecture Tree](https://github.com/marmelab/ArchitectureTree)
- [UMLet](https://www.umlet.com/)
- [GraphUI](https://github.com/artemanufrij/graphui)

### Markdown

- [Markdown Cheat sheet](https://www.markdownguide.org/cheat-sheet/)
- [Markwhen](https://docs.markwhen.com/interface/overview.html)
- [remarkable](https://remarkableapp.github.io/linux.html)

### Architecture

- [Topic Architecture Best Practices](https://docs.solace.com/Messaging/Topic-Architecture-Best-Practices.htm)
- [Everything You Need To Know About Microservices Design Patterns](https://www.edureka.co/blog/microservices-design-patterns#:~:text=Microservices%2C%20aka%20microservice%20architecture%2C%20is,implements%20a%20single%20business%20capability.)
- [Cloud Design Patterns](https://learn.microsoft.com/en-us/azure/architecture/patterns/)
- [Awesome Software and Architectural Design Patterns](https://github.com/DovAmir/awesome-design-patterns?utm_source=tldrnewsletter)
- [Awesome Design Patterns](https://github.com/DovAmir/awesome-design-patterns)
- [Domain-Driven Hexagon](https://github.com/Sairyss/domain-driven-hexagon)
- [Solution Architecture Patterns](https://github.com/chanakaudaya/solution-architecture-patterns)
- [Awesome Software Architecture](https://github.com/mehdihadeli/awesome-software-architecture)
- [3 Rs of Software Architecture](https://github.com/ryanmcdermott/3rs-of-software-architecture)
- [The Full-Stack Software Design and Architecture Roadmap](https://github.com/stemmlerjs/software-design-and-architecture-roadmap)

#### ADR

- [Architecture decision record (ADR)](https://github.com/joelparkerhenderson/architecture-decision-record)
- [Hugo Markdown ADR Tools](https://github.com/butonic/adr-tools)
- [Python ADR tools7](https://pypi.org/project/adr-tools-python/)
- [ADR Viewer](https://github.com/mrwilson/adr-viewer)

### Topics (MQTT)

- [mqtt-topic-tree-design-best-practices-tips-example](https://pi3g.com/2019/05/29/mqtt-topic-tree-design-best-practices-tips-examples/)
- [Describing Kafka with AsyncAPI](https://dalelane.co.uk/blog/?p=4219)
- [WebSocket vs. MQTT vs. CoAP: Which is the Best Protocol?](https://www.nabto.com/websocket-vs-mqtt-vs-coap/)

### GCP

- [Google Cloud Cheat Sheet](https://googlecloudcheatsheet.withgoogle.com/)
- [The Google Cloud Developer's Visual Notes](https://github.com/priyankavergadia/GCPSketchnote)
- [Write event-driven functions](https://cloud.google.com/functions/docs/writing/write-event-driven-functions)

### GUID

- [The Wild World of Unique Identifiers (UUID, ULID, etc)](https://medium.com/geekculture/the-wild-world-of-unique-identifiers-uuid-ulid-etc-17cfb2a38fce)
- [Globally Unique Identifier a fair comparison](https://medium.com/@orefalo_66733/globally-unique-identifier-a-fair-comparison-12b114c78ead)
- [ULID](https://github.com/ulid/spec)
- [CUID2](https://github.com/paralleldrive/cuid2)

## Examples

### API's

- [Octopus Energy APIs](https://developer.octopus.energy/docs/api/#list-tariff-charges)
- [Switchcraft energy API (1.1.0)](https://swc-api.switchcraft.co.uk/energy-api/api-docs/openapi/index.html#tag/Market-information/operation/getTariffDetails)

## Products

- [Energy System Efficiency Monitoring | Leverege IoT Use Case](https://www.leverege.com/usecases/energy-system-efficiency-monitoring)
- [Smart energy monitoring, data visualization and energy efficiency analysis | ThingsBoard](https://thingsboard.io/smart-energy/)
- [LoweConex](https://www.loweconex.com/net-zero-solutions/energy-reduction-via-iot/)
- [Unified Data Platform - LoweConex](https://www.loweconex.com/unified-data-platform/)
- [Sky Centrics](https://skycentrics.com/)

## IOT

### MQTT

- [Understanding MQTT Topics & Wildcards by Case](https://www.emqx.com/en/blog/advanced-features-of-mqtt-topics)
- [Beginners Guide To The MQTT Protocol](http://www.steves-internet-guide.com/mqtt/)
- [Comparing MQTT Brokers for the Industrial IoT](https://learn.umh.app/blog/comparing-mqtt-brokers-for-the-industrial-iot/)

### MQTT IoT

- [HiveMQ](https://www.hivemq.com/blog/best-alternative-google-cloud-iot-core-integrating-mqtt-data-with-gcp/)
- [ClearBlade](https://www.clearblade.com/)

### IoT Mobile

- [1nce](https://1nce.com/en-us/)
- [ThingsMobile](https://www.thingsmobile.com/business)
- [Emnify](https://www.emnify.com/)
- [InfiSIM](https://infisim.com/)
- [World OV](https://www.worldov.com/)
- [Emnify](https://www.emnify.com/)
- [Forest Rock](https://www.forestrock.co.uk/)
- [Scancom](https://www.scancom.co.uk/)

### IoT HW

- [RUT956 INDUSTRIAL CELLULAR ROUTER](https://teltonika-networks.com/products/routers/rut956)
- [ATOP - MB5901B Series 4G LTE Cellular to Ethernet and Serial Secure Modbus Gateway/Router](https://www.atoponline.com/industrial-networking-computing/products/serial-modbus-gateways/cellular-gateways/mb5901b/)
- [EasyNET 2G/4G NB-IoT data logger](https://www.4next.eu/en/prodotto/gsm-data-logger/)
- [Impulse Embedded](https://www.impulse-embedded.co.uk/)

### IoT platforms

- [Thingsboard](https://thingsboard.io/)
- [Thingsboard OTA](v/docs/pe/user-guide/ota-updates/)

## Top Tools

### Design tools

- [PlantUML](https://plantuml.com/)
- [PlantUMl vscode](https://marketplace.visualstudio.com/items?itemName=jebbs.plantuml)
- [Mermaid](https://mermaid.js.org/#/)
- [Mermaid VS code](https://marketplace.visualstudio.com/items?itemName=vstirbu.vscode-mermaid-preview)
- [Drawio](https://www.diagrams.net/)
- [Drawio vscode](https://marketplace.visualstudio.com/items?itemName=hediet.vscode-drawio)
- [Markdown](https://www.markdownguide.org/basic-syntax/)
- [Wavedrom - digital timing diagrams](https://wavedrom.com/)
- [WireViz - documenting cables, wiring harnesses and connector pinouts.](https://github.com/formatc1702/WireViz)
- [kroki](https://kroki.io/)
- [svgbob - text diagrams to svg](https://github.com/ivanceras/svgbob)
- [bytefield-svg create bytefield diagrams](https://github.com/Deep-Symmetry/bytefield-svg)
- [packetdiag](https://github.com/blockdiag/nwdiag/blob/master/packetdiag.1)
- [ditaa - text (ascii art) to svg](https://ditaa.sourceforge.net/)

### API
- [Widdrshins Open APi to Markdown](https://github.com/Mermade/widdershins)
- [panadoc markdown to html](https://pandoc.org/demos.html)
- [Postman](https://www.postman.com/downloads/)

### WSL

- [WSL](https://learn.microsoft.com/en-us/windows/wsl/install)

### Linux tools

- [GitUI provides you with the comfort of a git GUI but right in your terminal - Rust based app](https://github.com/extrawurst/gitui)
- [Based on OpenAPI specification, openapi-fuzzer provides random data as inputs to the API endpoints in order to find bugs.](https://github.com/matusf/openapi-fuzzer)
- [topgrade - Upgrade all of your tools](https://github.com/topgrade-rs/topgrade)
- [bottom A customizable cross-platform graphical process/system monitor for the terminal](https://github.com/ClementTsang/bottom)
- [Broot - is another rust tool, through which you can navigate directories.](https://github.com/Canop/broot)
- [Oh My Zsh](https://ohmyz.sh/)
- [exa - modern replacement for ls](https://the.exa.website/)
- [lsd - An ls with a lot of pretty colors and awesome icons](https://github.com/Peltoche/lsd)
- [mc Midnight Commander is a free cross-platform orthodox file manager](https://github.com/MidnightCommander/mc)
- [ncdu provides a useful and convenient way to view disk usage.](https://dev.yorhel.nl/ncdu)
- [bmon is a simple yet powerful, text-based network monitoring and debugging tool for Unix-like systems,](https://github.com/tgraf/bmon)
- [du-dust # Dust provides a nice alternative to the Linux du command.](https://github.com/bootandy/dust)
- [Onefetch is a command-line Git information tool written in Rust that displays project information and code statistics for a local Git repository](https://github.com/o2sh/onefetch)
- [git-delta - The delta tool (a.k.a. git-delta, a.k.a. delta-diff) is a diff viewer written in Rust](https://dandavison.github.io/delta/)

### Other

- [Regex Tester](https://github.com/artemanufrij/regextester)

## Development methodologies

### SAFe

- [What is SAFe](https://scaledagile.com/what-is-safe/)
- [The SAFe Delusion](https://safedelusion.com/)
- [The Major Problems with SAFe](https://productcoalition.com/the-major-problems-with-safe-1e797f7e48f8)
- [Issues with Safe](https://ronjeffries.com/xprog/articles/issues-with-safe/)
- [Beware SAFe (the Scaled Agile Framework for Enterprise), an Unholy Incarnation of Darkness](https://seandexter1.medium.com/beware-safe-the-scaled-agile-framework-for-enterprise-an-unholy-incarnation-of-darkness-bf6819f6943f)
- [L.A.F.A.B.L.E](https://www.lafable.com/)
- [A Critique of SAFe – The Scaled Agile Framework](https://tomgeraghty.co.uk/index.php/a-short-critique-of-safe/)

## Other (temp dumnping ground for links)

- [Enterprise Architecture Principles - dfe](https://dfe-digital.github.io/architecture/principles/enterprise-architecture-principles/#enterprise-architecture-principles)
- [The Technology Code of Practice - UK Gov](https://www.gov.uk/guidance/the-technology-code-of-practice)
- [5 Key Principles of Software Architecture](https://towardsdatascience.com/5-key-principles-of-software-architecture-e5379cb10fd5)
- [Azure - Architectural principles](https://learn.microsoft.com/en-us/dotnet/architecture/modern-web-apps-azure/architectural-principles)
- [Architectural Principles](https://cio-wiki.org/wiki/Architectural_Principles)
- [TOGAF - Architecture Principles](https://pubs.opengroup.org/architecture/togaf8-doc/arch/chap29.html)
- [IT Architecture Principles](https://www.itarch.info/2020/01/it-architecture-principles.html#:~:text=Architecture%20Principle%201%3A%20Reuse%20before,consider%20buying%20instead%20of%20building.)
- [Key Principles](https://sceweb.sce.uhcl.edu/helm/SWEN5233%20Software%20Architecture/my_files/TableContents/Module-3/key_principles.html)

- [Embedded Design Pattern Catalogue](https://embeddedartistry.com/fieldatlas/design-pattern-catalogue/)
- [Design Patterns](https://www.eventhelix.com/design-patterns/)
- [Embedded Control Systems Design/Design Patterns](https://en.wikibooks.org/wiki/Embedded_Control_Systems_Design/Design_Patterns)
- [NHS+Architecture+Principles+January+2021+v4+.docx](./NHS+Architecture+Principles+January+2021+v4+.docx)
- [Design_Patterns_for_Embedded_Systems_in_C.pdf](.\Design_Patterns_for_Embedded_Systems_in_C.pdf)
- [Embedded Control Systems Design/Design Patterns](https://en.wikibooks.org/wiki/Embedded_Control_Systems_Design/Design_Patterns)
- [The 7 Most Important Software Design Patterns](https://learningdaily.dev/the-7-most-important-software-design-patterns-d60e546afb0e)
- [Design Patterns](https://sourcemaking.com/design_patterns)
- [Top 5 Popular Software Design Patterns in 2023](https://www.upgrad.com/blog/software-design-patterns/)
